...
module.exports = function ({ templatesRoot, file, destRoot, answers = {}, forceOverride = false }, doneHook) {
  for (const key in answers) {
    if (typeof answers[key] === 'object') answers[key] = JSON.stringify(answers[key])
  }
  const sourcePath = file.path
  let destPath = path.join(destRoot, sourcePath.replace(templatesRoot, ''))
  ...
  if (sourcePath.indexOf('.gitignore') > -1 || sourcePath.indexOf('gitignore') > -1) {
    ...
    fs.readFile(sourcePath, function (err, sourceData) {
      ...
      fse.ensureFile(destPath, function (err) {
        ...
        fs.readFile(destPath, function (err, destData) {
          ...
          const sourceLines = sourceData.toString().split('\n')
          const destLines = destData.toString().split('\n')
    ...
    fs.readFile(sourcePath, function (err, data) {
      ...
      if (require('istextorbinary').isText(sourcePath, data)) {
        data = format(data.toString(), answers)
      }
      fse.pathExists(destPath, function (err, exists) {
        if (err) {
          console.error('failed to check existance of file', destPath, err)
          return doneHook(err)
        }
        if (exists && !forceOverride) {
          console.error('destination file exists and will not override', destPath)
          return doneHook(new Error('destination file exists'))
        }
        fse.ensureFile(destPath, function (err) {
          if (err) {
            console.error('failed to ensure file', destPath, err)
